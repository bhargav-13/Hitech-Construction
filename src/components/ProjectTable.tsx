    ...
    <div className="rounded-xl border border-gray-200 bg-white">
      ...
      <div className="w-full min-w-0 overflow-x-auto">
        <table className="w-full min-w-[1100px] border-collapse text-sm">
          <thead>
            <tr className="border-b border-indigo-100 bg-gray-50 text-left text-gray-500">
              <th className="border-r border-indigo-100 px-3 py-2 w-10">
                <LayoutList size={13} className="text-indigo-300" />
              </th>
              {COLUMNS.map((col) => (
                <th
                  key={col.label}
                  className="border-r border-indigo-100 px-3 py-2 font-medium whitespace-nowrap last:border-r-0"
                >
                  <span className="inline-flex items-center gap-1.5">
                    {col.icon === "date" ? (
                      <Calendar size={13} className="text-indigo-300" />
                    ) : (
                      <Type size={13} className="text-indigo-300" />
                    )}
                    {col.label}
          ...
          <tbody>
            {projects.map((project, i) => (
              <tr key={project.id} className="border-b border-indigo-50">
                <td className="border-r border-indigo-50 px-3 py-2 text-gray-400">{i + 1}</td>
                {COLUMNS.map((col) => (
                  <td
                    key={col.label}
                    className={`border-r border-indigo-50 px-3 py-2 whitespace-nowrap last:border-r-0 ${
                      col.key === "stage" || col.key === "name" ? "text-indigo-600" : "text-gray-700"
                    }`}
                  >
